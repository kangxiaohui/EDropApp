...
import net.edrop.edrop_user.adapter.HorizontalRecyclerviewAdapter;
import net.edrop.edrop_user.adapter.NoHorizontalScrollerVPAdapter;
import net.edrop.edrop_user.emotionkeyboardview.EmotionKeyboard;
import net.edrop.edrop_user.emotionkeyboardview.NoHorizontalScrollerViewPager;
import net.edrop.edrop_user.model.ImageModel;
import net.edrop.edrop_user.utils.EmotionUtils;
import net.edrop.edrop_user.utils.GlobalOnItemClickManagerUtils;
...
public class EmotionMainFragment extends BaseFragment {
    ...
    protected void initDatas() {
        ...
        //记录底部默认选中第一个
        CurrentPosition = 0;
        SharedPreferencesUtils.setInteger(getActivity(), CURRENT_POSITION_FLAG, CurrentPosition);
        ...
        horizontalRecyclerviewAdapter.setOnClickItemListener(new HorizontalRecyclerviewAdapter.OnClickItemListener() {
            ...
            public void onItemClick(View view, int position, List<ImageModel> datas) {
                //获取先前被点击tab
                int oldPosition = SharedPreferencesUtils.getInteger(getActivity(), CURRENT_POSITION_FLAG, 0);
                //修改背景颜色的标记
